...
import com.tiktoknew.activitesfragments.livestreaming.stats.StatsManager;
import com.tiktoknew.activitesfragments.livestreaming.ui.VideoGridContainer;
import com.tiktoknew.activitesfragments.sendgift.StickerGiftF;
...
import com.tiktoknew.models.UserOnlineModel;
        ...
        StickerGiftF giftFragment = new StickerGiftF(item.getUserId(), item.getUserName(), item.getUserPicture(), new FragmentCallBack() {
            ...
            public void onResponce(Bundle bundle) {
                if (bundle.getBoolean("isShow",false))
                {
                    ...
                    Log.d(com.tiktoknew.Constants.tag,"Test : "+streamerLiveModel.getUserCoins());
...
//                            tabJoinVideo.setImageDrawable(ContextCompat.getDrawable(context,R.drawable.ic_non_duet_post_new));
                    ...
                    if (!(TextUtils.isEmpty(snapshot.getValue().toString())))
                    {
                       SingleCastJoinA.this.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (item.getUserId().equalsIgnoreCase(itemUpdate.getUserId()))
                                {
                                    Functions.showIndeterminentLoader(context,itemUpdate.getUserName()+" "+context.getString(R.string.single_is_week)
                                            ,false,false);
                                    timer.cancel();
                                    timer = new Timer();
                                    timer.schedule(
    ...
    {
        return com.tiktoknew.activitesfragments.livestreaming.Constants.VIDEO_DIMENSIONS[
